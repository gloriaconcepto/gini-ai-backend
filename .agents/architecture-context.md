...
The frontend presentation layer consists of 2 isolated single-page application (SPA) Container Apps deployed with external HTTPS ingress on Azure Container Apps:
- **Unified Enterprise Portal (`ca-enterprise-portal`):** Consolidated application serving `/`, `/user/*`, `/manager/*`, and `/admin/*` on Port 3001, unifying tenant workspace operations, IAM, Maker-Checker governance, and intelligence interfaces.
- **OEM Backoffice (`ca-oem-backoffice`):** OEM administration and white-label governance console serving `/`, `/dashboard`, and `/tenants` on Port 3002.